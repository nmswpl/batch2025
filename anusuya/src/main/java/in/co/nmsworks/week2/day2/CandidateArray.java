...
public class CandidateArray {
    ...
    public static void printCandidate(Candidate[] candidates){

        for(int i = 0; i < candidates.length; i++){
            System.out.println(candidates[i].getId() + " " + candidates[i].getName() +" " +  candidates[i].getEmail() + " " +candidates[i].getDepartment());
        }

        for(Candidate Candidate : candidates){
            System.out.println(Candidate.getId() + " " + Candidate.getName() + " " + Candidate.getEmail() + " " + Candidate.getDepartment());
        }

        for(Candidate candidate : candidates){
            if(candidate.getDepartment().equals("QA")){
